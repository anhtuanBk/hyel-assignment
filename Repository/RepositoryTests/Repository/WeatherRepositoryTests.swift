...
class WeatherRepositoryTests: XCTestCase {
    ...
    private var dailyWeatherDao: DailyWeatherDaoMock!
    private var hourlyWeatherDao: HourlyWeatherDaoMock!
    private var weatherApi: WeatherApiMock!
    private var weatherRepository: WeatherRepository!
    private var cancellables: Set<AnyCancellable>!
    
    private var testingDays: [Day]!
    
    private let fakeDailyWeatherData = """
    ...
    override func setUpWithError() throws {
        ...
        let data = try! decoder.decode(DailyWeather.self, from: Data(fakeDailyWeatherData.utf8))
        testingDays = data.forecastDaily.days
        
        cancellables = Set<AnyCancellable>()
        dailyWeatherDao = mock(DailyWeatherDao.self)
        hourlyWeatherDao = mock(HourlyWeatherDao.self)
        weatherApi = mock(WeatherApi.self)
        weatherRepository = WeatherRepositoryImpl(weatherApi: weatherApi, dailyWeatherDao: dailyWeatherDao, hourlyWeatherDao: hourlyWeatherDao)
    }
    ...
    func testFetchDailyWeatherWithoutCache() throws {
        // when
        let location = CLLocationCoordinate2D(latitude: 0, longitude: 0)
        given(weatherApi.fetchWeatherDaily(location.latitude, location.longitude)).willReturn(
            Future { [unowned self] promise in
                promise(.success(self.testingDays))
            }
                .eraseToAnyPublisher()
        )
        
        given(dailyWeatherDao.getDailyWeathers(location.latitude, location.longitude)).willReturn(nil)
        ...
        weatherRepository
            .fetchDailyWeather(location.latitude, location.longitude)
            .sink { completion in
                switch completion {
                case .finished:
                    break
